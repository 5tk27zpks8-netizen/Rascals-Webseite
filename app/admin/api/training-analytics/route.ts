...
export async function GET(request:Request){
 ...
 const sessions=sessionsRes.results.map(r=>({id:String(r.id),title:String(r.title??"Training"),date:String(r.starts_at??r.created_at??""),duration:Number(r.duration_minutes??0),status:String(r.status??"draft"),completion:r.completion_pct==null?null:Number(r.completion_pct),execution:r.execution_rating==null?null:Number(r.execution_rating),intensity:r.intensity_rating==null?null:Number(r.intensity_rating),objectiveMet:r.objective_met==null?null:Number(r.objective_met)===1}));
 const completed=sessions.filter(s=>s.status==="completed");const completedIds=completed.map(s=>s.id);

 let periods:Row[]=[];
 if(completedIds.length){const marks=completedIds.map(()=>"?").join(",");const p=await DB.prepare(`SELECT tp.*,ts.starts_at,ts.created_at session_created_at FROM training_periods tp JOIN training_sessions ts ON ts.id=tp.session_id WHERE tp.session_id IN (${marks}) ORDER BY COALESCE(ts.starts_at,ts.created_at),tp.sort_order`).bind(...completedIds).all<Row>();periods=p.results}
 ...
 const bySkill=new Map<string,{skillKey:string;minutes:number;sessions:Set<string>;group:string;unit:string}>();
 ...
 for(const p of periods){const min=Number(p.minutes??0),group=String(p.position_group??"TEAM")||"TEAM",unit=String(p.unit??"team")||"team",key=String(p.source_key??"");
   ...
   if(String(p.source_type)==="development-need"&&key){const x=bySkill.get(key)??{skillKey:key,minutes:0,sessions:new Set<string>(),group,unit};x.minutes+=min;x.sessions.add(String(p.session_id));bySkill.set(key,x)}
 }
 ...
 const evals=await DB.prepare("SELECT player_id,skill_key,current_rating,evaluated_at FROM player_skill_evaluations WHERE season_id=? ORDER BY evaluated_at ASC,created_at ASC").bind(season.id).all<Row>();
 ...
 const effectiveness=[...bySkill.values()].map(x=>{const rows=evalBySkill.get(x.skillKey)??[];const firstByPlayer=new Map<string,number>(),lastByPlayer=new Map<string,number>();for(const e of rows){const pid=String(e.player_id),v=Number(e.current_rating??0);if(!firstByPlayer.has(pid))firstByPlayer.set(pid,v);lastByPlayer.set(pid,v)}const deltas=[...lastByPlayer.entries()].map(([pid,last])=>firstByPlayer.has(pid)?last-Number(firstByPlayer.get(pid)):null).filter((v):v is number=>v!=null);const def=defMap.get(x.skillKey);return{skillKey:x.skillKey,skillLabel:def?.label??x.skillKey,group:def?.group||x.group,category:def?.category??"",minutes:x.minutes,sessions:x.sessions.size,evaluatedPlayers:deltas.length,skillDelta:deltas.length?r1(avg(deltas)):null,effectivenessPer60:deltas.length&&x.minutes>0?r1((avg(deltas)*60)/x.minutes):null}}).sort((a,b)=>b.minutes-a.minutes);

 const attendanceRows=await DB.prepare(`SELECT occurred_at,attendance FROM player_performance_entries WHERE context='training' AND substr(occurred_at,1,4)=? ORDER BY occurred_at`).bind(String(season.year)).all<Row>();
 ...
 for(const s of completed){const m=monthKey(s.date),x=monthMap.get(m)??{month:m,sessions:0,minutes:0,present:0,totalAttendance:0,execution:[],intensity:[]};x.sessions+=1;if(s.execution!=null)x.execution.push(s.execution);if(s.intensity!=null)x.intensity.push(s.intensity);monthMap.set(m,x)}
 for(const p of periods){const m=monthKey(String(p.starts_at??p.session_created_at??"")),x=monthMap.get(m)??{month:m,sessions:0,minutes:0,present:0,totalAttendance:0,execution:[],intensity:[]};x.minutes+=Number(p.minutes??0);monthMap.set(m,x)}
 for(const a of attendanceRows.results){const m=monthKey(String(a.occurred_at??"")),x=monthMap.get(m)??{month:m,sessions:0,minutes:0,present:0,totalAttendance:0,execution:[],intensity:[]};const st=String(a.attendance??"none");if(["present","absent","excused","injured"].includes(st)){x.totalAttendance++;if(st==="present")x.present++}monthMap.set(m,x)}

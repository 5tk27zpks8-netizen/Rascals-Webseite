...
type GroupNeed = SkillBase & { count: number; current: number; target: number; priority: number; confidence: number; needScore: number; coveragePct: number; trainingScore: number; trend: number | null; state: "evaluate-first" | "focus" | "improve" | "maintain" };
type Skill = SkillBase & { current: number; target: number; priority: number; confidence: number; note: string; needScore: number; delta: number | null; lastEvaluatedAt: string | null; lastEvaluatedBy: string; history: Evaluation[] };
...
export function PositionDevelopmentManager() {
  ...
  return <AdminShell active="positiondev" title="Position Development" eyebrow="RASCALS OS · GENERIC DEVELOPMENT FRAMEWORK" actions={<><a className="cms-button secondary" href="/admin/development/linebackers">LB Lab</a><a className="cms-button secondary" href="/admin/stats/analytics">Stats Analytics</a></>}>
    ...
    <AdminCard className="pd-hero">
      <div><small>ONE ENGINE · MULTIPLE POSITION ROOMS</small><h2>{data?.group.label ?? "Position Development"}</h2><p>QB, RB, WR/TE, OL, DL, LB, DB und Special Teams nutzen dieselbe Evaluation-, Goal-, Trend- und Evidence-Architektur. Positionsspezifisch ändern sich Skill-Modell und relevante Daten.</p></div>
    ...
    {mode === "room" && data && <>
      <div className="pd-room-grid">
        <AdminCard><div className="cms-section-head"><div><small>POSITION NEED MATRIX</small><h2>Top Entwicklungsfelder</h2></div></div><div className="pd-needs">{topNeeds.map((need, index) => <article key={need.key}><b>{index + 1}</b><div><strong>{need.label}</strong><span>{need.category} · {need.count}/{data.players.length} bewertet</span></div><em>{need.current || "–"} → {need.target || "–"}</em><Status state={need.state} score={need.trainingScore}/><strong>{need.coveragePct}%</strong></article>)}{!topNeeds.length && <p className="cms-muted">Noch keine Evaluationen für diese Positionsgruppe.</p>}</div></AdminCard>
      ...
      <AdminCard><div className="cms-section-head"><div><small>GROUP MATRIX</small><h2>{data.group.label}</h2></div><span className="cms-muted">Latest Evaluation pro Player + Skill</span></div><div className="pd-table-wrap"><table className="pd-table"><thead><tr><th>Skill</th><th>Current</th><th>Target</th><th>Need</th><th>Training</th><th>Coverage</th><th>Trend</th></tr></thead><tbody>{data.groupNeeds.map((need) => <tr key={need.key}><td><b>{need.label}</b><small>{need.category}</small></td><td>{need.count ? need.current : "–"}</td><td>{need.count ? need.target : "–"}</td><td>{need.count ? need.needScore : "–"}</td><td><Status state={need.state} score={need.trainingScore}/></td><td>{need.coveragePct}%</td><td><Trend value={need.trend}/></td></tr>)}</tbody></table></div></AdminCard>
    </>}
    ...
    {mode === "player" && data?.player && <>
      ...
      <div className="pd-skill-grid">{data.player.skills.map((skill) => {
        ...
        return <AdminCard key={skill.key} className="pd-skill"><div className="pd-skill-head"><div><small>{skill.category}</small><h3>{skill.label}</h3><p>{skill.description}</p></div><b className={need >= 50 ? "hot" : ""}>{need}</b></div><div className="pd-ratings"><label>IST<select value={draft.current} onChange={(event) => patch(skill.key, "current", Number(event.target.value))}>{[1, 2, 3, 4, 5].map((value) => <option key={value}>{value}</option>)}</select></label><label>ZIEL<select value={draft.target} onChange={(event) => patch(skill.key, "target", Number(event.target.value))}>{[1, 2, 3, 4, 5].map((value) => <option key={value}>{value}</option>)}</select></label><label>PRIO<select value={draft.priority} onChange={(event) => patch(skill.key, "priority", Number(event.target.value))}>{[1, 2, 3].map((value) => <option key={value}>{value}</option>)}</select></label><label>CONF<select value={draft.confidence} onChange={(event) => patch(skill.key, "confidence", Number(event.target.value))}>{[1, 2, 3, 4, 5].map((value) => <option key={value}>{value}</option>)}</select></label></div><label className="cms-field"><span>Coach Note / Evidence</span><textarea rows={2} value={draft.note} onChange={(event) => patch(skill.key, "note", event.target.value)}/></label><div className="pd-skill-foot"><span><Trend value={skill.delta}/>{skill.lastEvaluatedAt ? ` · ${formatDate(skill.lastEvaluatedAt)}` : " · noch keine Evaluation"}</span><button className="cms-button" disabled={saving === skill.key} onClick={() => void saveEvaluation(skill)}>{saving === skill.key ? "Speichert…" : "Evaluation speichern"}</button></div></AdminCard>;
      })}</div>
    </>}

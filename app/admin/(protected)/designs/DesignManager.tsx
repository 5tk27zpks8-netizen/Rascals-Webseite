...
export function DesignManager(){
  ...
  async function apply(preset:DesignPreset){if(!state||!page||status==="saving")return;const previous=state;const next=applyDesignPreset(state,page.id,preset);setBeforeState(previous);setState(next);setStatus("saving");setNotice("");try{await persistBuilder(next);setPreviewPreset(null);setNotice(`„${preset.name}“ wurde als Website-Entwurf auf „${page.name}“ übernommen. Deine Inhalte bleiben erhalten.`);setStatus("done");window.setTimeout(()=>setStatus("idle"),1200)}catch(error){setState(previous);setBeforeState(null);setNotice(error instanceof Error?error.message:"Design konnte nicht angewendet werden.");setStatus("idle")}}
  ...
  return <AdminShell active="designs" title="Designs" eyebrow="RASCALS DESIGN LIBRARY" actions={<a className="cms-button secondary" href="/admin/website">Website gestalten</a>}>
    ...
    <div className="design-studio">
      ...
      <div className="design-section-title"><small>KOMPLETTE KONZEPTE</small><h2>Design auswählen</h2><p>Erst ansehen, dann übernehmen. Die Vorschau verwendet deine bestehenden Inhalte.</p></div>
    ...
    {previewPreset&&<div className="design-preview-modal-backdrop" onMouseDown={()=>setPreviewPreset(null)}><div className="design-preview-modal" onMouseDown={event=>event.stopPropagation()}><header><div><small>DESIGN VORSCHAU</small><h2>{previewPreset.name}</h2><p>{previewPreset.description}</p></div><button onClick={()=>setPreviewPreset(null)}>×</button></header><div className="design-preview-browser"><DesignWebsitePreview state={state} page={page} preset={previewPreset} full/></div><footer><button onClick={()=>setPreviewPreset(null)}>Zurück</button><button className="primary" disabled={status==="saving"} onClick={()=>void apply(previewPreset)}>{status==="saving"?"Wird übernommen…":"Design als Entwurf übernehmen"}</button></footer></div></div>}

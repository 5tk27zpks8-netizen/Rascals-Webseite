  ...
  return <AdminShell active="gamedayroster" title="Gameday Roster" actions={<>
    ...
    <AdminCard className="gdr-game-card">
      <div className="gdr-game-head">
        ...
        <div className="gdr-matchup"><img src="/rascals-logo-768.webp" alt=""/><span><small>{data?.game.seasonName||currentGame?.seasonName||"SAISON"}</small><b>RASCALS <i>VS</i> {data?.game.opponent||currentGame?.opponent||"—"}</b><em>{formatDate(data?.game.kickoff||currentGame?.kickoff||null)}</em></span>{data?.game.opponentLogo||currentGame?.opponentLogo?<img src={data?.game.opponentLogo||currentGame?.opponentLogo} alt=""/>:<div className="gdr-opponent-placeholder">?</div>}</div>

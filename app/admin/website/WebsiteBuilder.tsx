...
const iconLibrary = [
  ["Football farbig","https://api.iconify.design/noto/american-football.svg"],
  ["Football Outline","https://api.iconify.design/ion/american-football-outline.svg?color=%23e7192d"],
...
export function WebsiteBuilder(){
  ...
  async function load(){
    try{
      ...
      setState(builder.state);setSavedSnapshot(JSON.stringify(builder.state));setMedia(mediaBody.items||[]);
  ...
  useEffect(()=>{const handler=(event:KeyboardEvent)=>{if((event.metaKey||event.ctrlKey)&&event.key.toLowerCase()==="s"){event.preventDefault();if(dirty&&!saving&&!duplicateSlug)void save()}};window.addEventListener("keydown",handler);return()=>window.removeEventListener("keydown",handler)},[dirty,saving,duplicateSlug,state]);
  ...
  function createPage(){if(!state)return;const name=newPageName.trim()||"Neue Seite";const base=slugify(name)||`seite-${state.pages.length+1}`;let slug=base;let count=2;while(state.pages.some(item=>item.slug===slug)){slug=`${base}-${count++}`}const first=freshSection("hero");const next:BuilderPage={id:crypto.randomUUID(),name,slug,enabled:false,showInNav:false,navLabel:name,title:name,description:"",sections:[first]};setState({...state,pages:[...state.pages,next]});setPageId(next.id);setSectionId(first.id);setInspectorMode("page");setShowNewPage(false);setNewPageName("")}
  function duplicatePage(){if(!state||!page)return;const base=`${page.slug||"startseite"}-kopie`;let slug=base;let count=2;while(state.pages.some(item=>item.slug===slug)){slug=`${base}-${count++}`}const copy:BuilderPage={...page,id:crypto.randomUUID(),name:`${page.name} Kopie`,slug,enabled:false,showInNav:false,sections:page.sections.map(item=>({...item,id:crypto.randomUUID(),items:item.items.map(child=>({...child,id:crypto.randomUUID()}))}))};setState({...state,pages:[...state.pages,copy]});setPageId(copy.id);setSectionId(copy.sections[0]?.id||"");setInspectorMode("page")}
  function deletePage(){if(!state||!page||page.slug==="")return;if(!confirm(`Seite „${page.name}“ wirklich löschen?`))return;const pages=state.pages.filter(item=>item.id!==page.id);setState({...state,pages});const next=pages[0];setPageId(next?.id||"");setSectionId(next?.sections[0]?.id||"");setInspectorMode(next?.sections.length?"section":"page")}
  function movePage(direction:-1|1){if(!state||!page)return;const index=state.pages.findIndex(item=>item.id===page.id);const target=index+direction;if(target<0||target>=state.pages.length)return;const pages=[...state.pages];[pages[index],pages[target]]=[pages[target],pages[index]];setState({...state,pages})}
  ...
  async function save(){if(!state||duplicateSlug)return;setSaving(true);setNotice("");try{const response=await fetch("/admin/api/site-builder",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(state)});const body=await response.json().catch(()=>({})) as {error?:string};if(!response.ok)throw new Error(body.error||"Speichern fehlgeschlagen");setSavedSnapshot(JSON.stringify(state));setNotice("Gespeichert. Veröffentlichte Seiten sind jetzt live.")}catch(error){setNotice(error instanceof Error?error.message:"Speichern fehlgeschlagen")}finally{setSaving(false)}}
  ...
  return <AdminShell active="website" title="Website gestalten" eyebrow="RASCALS STUDIO" actions={<div className="wb-save-cluster"><span className={`wb-save-state ${dirty?"dirty":""}`}>{dirty?"Ungespeichert":"Gespeichert"}</span><button className="cms-button" disabled={saving||!dirty||duplicateSlug} onClick={()=>void save()}>{saving?"Speichert…":"Speichern"}</button></div>}>
    ...
    <div className="wb-studio">
      <aside className="wb-navigator">
        ...
        <button className={`wb-nav-special ${inspectorMode==="theme"?"active":""}`} onClick={()=>setInspectorMode("theme")}><span>◐</span><b>Website Design</b><small>Brand, Farben & Header</small></button>
        <button className="wb-add-main" onClick={()=>setShowLibrary(true)}>＋ Abschnitt hinzufügen</button>
      </aside>

      <main className="wb-canvas-column"><div className="wb-canvas-toolbar"><div><span className={page.enabled?"live":""}/><b>{page.name}</b><small>{page.enabled?"Veröffentlicht":"Entwurf"}</small></div><div className="wb-device-switch">{(["desktop","tablet","mobile"] as PreviewMode[]).map(mode=><button key={mode} className={previewMode===mode?"active":""} onClick={()=>setPreviewMode(mode)}>{mode==="desktop"?"Desktop":mode==="tablet"?"Tablet":"Mobil"}</button>)}</div><a href={page.slug?`/${page.slug}`:"/"} target="_blank">Öffnen ↗</a></div><div className="wb-canvas-shell"><div className={`wb-canvas ${previewMode}`}>{page.sections.length?page.sections.map(item=><div key={item.id} className={`wb-canvas-section ${inspectorMode==="section"&&item.id===section?.id?"selected":""}`} onClickCapture={event=>{event.preventDefault();event.stopPropagation();setSectionId(item.id);setInspectorMode("section")}}><BuilderSectionView section={item}/><span className="wb-selection-label">{typeLabel(item.type)}</span></div>):<button className="wb-empty-page" onClick={()=>setShowLibrary(true)}><b>Deine Seite ist leer</b><span>Ersten Abschnitt hinzufügen</span></button>}</div></div></main>

      <aside className="wb-inspector">
        {inspectorMode==="page"&&<PageInspector page={page} patchPage={patchPage} movePage={movePage} duplicatePage={duplicatePage} deletePage={deletePage}/>} 
        {inspectorMode==="theme"&&<ThemeInspector state={state} setState={setState} media={media} uploading={uploading} uploadImage={uploadImage}/>} 

...
export function DesignManager(){
  ...
  async function persistBuilder(next:SiteBuilderState){const response=await fetch("/admin/api/site-builder",{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(next)});const body=await response.json().catch(()=>({})) as{error?:string};if(!response.ok)throw new Error(body.error||"Design konnte nicht als Entwurf gespeichert werden.")}
  async function apply(preset:DesignPreset){if(!state||!page||status==="saving")return;const previous=state;const styled=applyDesignPreset(state,page.id,preset);const next=applyGlobalTheme?styled:{...styled,theme:state.theme};setBeforeState(previous);setState(next);setStatus("saving");setNotice("");try{await persistBuilder(next);setNotice(`„${preset.name}“ wurde als Entwurf auf „${page.name}“ angewendet${applyGlobalTheme?" – inklusive globalem Header und Grundfarben":""}. Veröffentlichen kannst du anschließend im Website Studio.`);setStatus("done");window.setTimeout(()=>setStatus("idle"),1200)}catch(error){setState(previous);setBeforeState(null);setNotice(error instanceof Error?error.message:"Design konnte nicht angewendet werden.");setStatus("idle")}}
  async function undo(){if(!beforeState||status==="saving")return;const restore=beforeState,current=state;setState(restore);setStatus("saving");try{await persistBuilder(restore);setBeforeState(null);setNotice("Letzte Design-Änderung wurde im Entwurf rückgängig gemacht.")}catch(error){if(current)setState(current);setNotice(error instanceof Error?error.message:"Rückgängig konnte nicht gespeichert werden.")}finally{setStatus("idle")}}
  ...
  return <AdminShell active="designs" title="Designs" eyebrow="RASCALS DESIGN LIBRARY" actions={<a className="cms-button secondary" href="/admin/website">Website gestalten</a>}>
    <div className="design-studio">
      <header className="design-hero"><div><small>DESIGN-BIBLIOTHEK</small><h2>Deine Website.<br/><span>Nur anders gestylt.</span></h2><p>Jede Vorschau zeigt jetzt deine tatsächlich ausgewählte Seite mit ihren aktuellen Bildern, Texten und Abschnitten. Geändert werden in der Vorschau nur die Farben und Designregeln des jeweiligen Looks.</p></div><div className="design-page-picker"><label><span>Vorschau-Seite</span><select value={page.id} onChange={event=>setPageId(event.target.value)}>{state.pages.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="design-global-option"><input type="checkbox" checked={applyGlobalTheme} onChange={event=>setApplyGlobalTheme(event.target.checked)}/><span><b>Globales Design übernehmen</b><small>Ändert zusätzlich Header und Grundfarben im Entwurf.</small></span></label><button className="design-save-own" onClick={()=>{setDesignName(`${page.name} Design`);setDesignDescription("");setShowSave(true)}}>＋ Eigenes Design speichern</button></div></header>
      ...
      <div className="design-filter" role="tablist"><button className={filter==="all"?"active":""} onClick={()=>setFilter("all")}>Alle</button><button className={filter==="built-in"?"active":""} onClick={()=>setFilter("built-in")}>Vorlagen</button><button className={filter==="custom"?"active":""} onClick={()=>setFilter("custom")}>Meine Designs <span>{custom.length}</span></button></div>
      <div className="design-grid">{presets.map(preset=><article className="design-card" key={preset.id}><div className="design-preview"><DesignWebsitePreview state={state} page={page} preset={preset}/><div className="design-swatches">{preset.palette.slice(0,4).map((color,index)=><span key={`${color}-${index}`} style={{background:color}}/>)}</div></div><div className="design-card-copy"><div className="design-card-title"><div><small>{preset.builtin?"VORLAGE":"EIGENES DESIGN"}</small><h3>{preset.name}</h3></div>{!preset.builtin&&<button className="design-delete" onClick={()=>void removeCustom(preset.id)}>×</button>}</div><p>{preset.description}</p><button className="design-apply" disabled={status==="saving"} onClick={()=>void apply(preset)}>{status==="saving"?"Bitte warten…":`In ${page.name} übernehmen`} <span>→</span></button></div></article>)}</div>

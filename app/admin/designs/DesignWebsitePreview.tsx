...
export function DesignWebsitePreview({state,page,preset,full=false}:{state:SiteBuilderState;page:BuilderPage;preset?:DesignPreset;full?:boolean}) {
  ...
  const scale=full?.78:.5;
  const width=full?`${100/scale}%`:"200%";
  const minHeight=full?1400:500;
  const viewportHeight=full?"72vh":250;
  return <div style={{height:viewportHeight,overflow:full?"auto":"hidden",background:theme.background,position:"relative",pointerEvents:full?"auto":"none"}}>
    <div style={{width,transform:`scale(${scale})`,transformOrigin:"top left",background:theme.background,color:theme.text,minHeight,
      ...
      <div style={{height:62,background:theme.headerBackground,borderBottom:"1px solid rgba(255,255,255,.1)",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 42px",color:theme.headerText}}>
        ...
        <div style={{display:"flex",gap:24,fontSize:11,fontWeight:800}}>{previewState.pages.filter(item=>item.showInNav).slice(0,6).map(item=><span key={item.id}>{item.navLabel}</span>)}</div>
      </div>
      {previewPage.sections.filter(section=>section.visible).map(section=><BuilderSectionView key={section.id} section={section}/>)}
    </div>
    {!full&&<div style={{position:"absolute",inset:0,boxShadow:"inset 0 0 0 1px rgba(255,255,255,.08)"}}/>}
  </div>;
}

...
export function HeroManager() {
  ...
  async function load() {
    ...
    try {
      ...
      const normalized = { ...cms.state, slides: cms.state.slides.map((slide) => ({ active: true, startsAt: null, endsAt: null, ...slide })) };
      setState(normalized);
  ...
  return (
    <AdminShell
      active="hero"
      title="Hero verwalten"
      actions={<button className="cms-button" disabled={saving || loading} onClick={() => void save()}>{saving ? "Speichert…" : "Änderungen speichern"}</button>}
    >
            ...
            <div className="hero-sort-list">
              {state.slides.map((slide, index) => (
                <button
                  ...
                  onDrop={(event) => dropSlide(index, event)}
                  onClick={() => setSelectedId(slide.id)}
                  className={selected?.id === slide.id ? "active" : ""}
                >
                  <span className="drag-handle">☰</span>
                  <img src={slide.image} alt="" />
                  <span className="hero-list-copy"><b>{slide.title} {slide.accent}</b><small>{slide.active === false ? "INAKTIV" : "AKTIV"} · Position {index + 1}</small></span>
                </button>
              ))}
            </div>
          </AdminCard>
          ...
          {selected && <div className="hero-editor-column">
            <AdminCard>
              ...
              <div className="cms-grid-2">
                ...
                <label className="cms-field"><span>Bild</span><select value={selected.image} onChange={(e) => patchSlide(selected.id, { image: e.target.value })}><option value={selected.image}>Aktuelles Bild</option>{media.map((item) => <option key={item.key} value={item.url}>{item.key.split("/").pop()}</option>)}</select></label>
                ...
                <label className="cms-field"><span>Button-Text</span><input value={selected.buttonLabel} onChange={(e) => patchSlide(selected.id, { buttonLabel: e.target.value })} /></label>
                <label className="cms-field"><span>Button-Link</span><input value={selected.buttonUrl} onChange={(e) => patchSlide(selected.id, { buttonUrl: e.target.value })} /></label>
              </div>
            </AdminCard>
            ...
            <AdminCard>
              ...
              <div className={`hero-live-frame ${previewMode}`}>
                <img src={selected.image} alt="" />
                <div className="hero-live-shade" />
                <div className="hero-live-copy"><small>{selected.eyebrow}</small><h2>{selected.title}<br/><i>{selected.accent}</i></h2><p>{selected.text}</p><span>{selected.buttonLabel} →</span></div>
              </div>
            </AdminCard>
            ...
            <AdminCard>
              <div className="cms-grid-2">
                ...
                <label className="cms-field"><span>Übergang</span><select value={state.transition} onChange={(e) => setState({ ...state, transition: e.target.value as CmsState["transition"] })}><option value="fade">Fade</option><option value="slide">Slide</option></select></label>
              </div>
            </AdminCard>
          </div>}
        </div>

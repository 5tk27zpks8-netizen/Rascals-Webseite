...
const topModules: TopModule[] = [
  ...
  {label:"WEBSITE",href:"/admin/hero",sections:["hero","news","media","sponsors"],actions:[{label:"Startseite",href:"/admin/hero"},{label:"News",href:"/admin/news"},{label:"Medien",href:"/admin/media"},{label:"Sponsoren",href:"/admin/sponsors"}]},
...
export function AdminShell({active,title,eyebrow="RASCALS WEBSITE",actions,children}:AdminShellProps){
  ...
  return <div className="cms-shell">
    <aside className="cms-sidebar">
      <a className="cms-brand" href="/admin/dashboard"><img src="/rascals-logo-transparent-4k.png" alt=""/><span><b>RASCALS</b><small>WEBSITE ADMIN</small></span></a>
      <nav>{navGroups.map(group=><div className="cms-nav-group" key={group.label}><p>{group.label}</p>{group.items.map(item=><a key={item.href+item.label} className={item.key===active||item.matches?.includes(active)?"active":""} href={item.href}><span>{item.icon}</span>{item.label}</a>)}</div>)}</nav>
      <div className="cms-user"><span>CG</span><div><b>CMS Zugriff</b><small>Website Verwaltung</small></div></div>
    </aside>
    <main className="cms-main">
      <header className="cms-topbar"><div><small>{eyebrow}</small><h1>{title}</h1></div><div className="cms-top-actions"><a href="/" target="_blank">Website öffnen ↗</a>{actions}</div></header>
      <nav className="cms-modulebar" aria-label="Hauptbereiche">{topModules.map(module=><a key={module.label} className={module.sections.includes(active)?"active":""} href={module.href}>{module.label}</a>)}</nav>
      <div className="cms-contextbar"><div className="cms-context-title"><small>{currentModule.label}</small><strong>Funktionen</strong></div><div className="cms-context-actions">{currentModule.actions.map(item=><a key={item.href+item.label} href={item.href}>{item.label}</a>)}</div></div>

...
export default async function NewsPage({ searchParams }: { searchParams: Promise<{ q?: string; category?: string; page?: string }> }) {
  ...
  return <>
    ...
    {/* A placeholder is not a label: it vanishes the moment anything is typed,
        and a screen reader may never announce it at all. Both fields are named
        out loud instead. `type="search"` gives a phone the search keyboard and
        the clear button its users expect, and `enterKeyHint` puts "Suchen" on
        the return key rather than a generic tick. */}
    <form className="public-news-filters" method="get" role="search"><input type="search" name="q" defaultValue={q} aria-label="News durchsuchen" enterKeyHint="search" placeholder="News durchsuchen…"/><select name="category" defaultValue={category} aria-label="Nach Kategorie filtern"><option value="">Alle Kategorien</option>{categories.map((item)=><option key={item}>{item}</option>)}</select><button type="submit">Filtern</button></form>

...
const footballIcon = "https://api.iconify.design/ion/american-football-outline.svg?color=%23ffffff";
...
function BuilderSectionView({ section }: { section: BuilderSection }) {
  ...
  if (section.type === "stats") return <section className="sb-section" style={vars}><div className="sb-inner"><div className="sb-section-head"><span className="sb-kicker">{section.eyebrow}</span><h2>{section.title}</h2></div><div className="sb-stats">{section.items.map((item)=><article key={item.id}><b>{item.value}</b><strong>{item.title}</strong><span>{item.text}</span></article>)}</div></div></section>;

  if (section.type === "cards") return <section className="sb-section" style={vars}><div className="sb-inner"><div className="sb-section-head"><span className="sb-kicker">{section.eyebrow}</span><h2>{section.title}</h2><p>{section.text}</p></div><div className="sb-cards">{section.items.map((item)=><article key={item.id}>{item.image&&<img src={item.image} alt=""/>}{item.icon&&<img className="sb-card-icon" src={item.icon} alt=""/>}<div><small>{item.eyebrow}</small><h3>{item.title}</h3><p>{item.text}</p>{item.linkLabel&&item.linkUrl&&<a href={item.linkUrl}>{item.linkLabel} →</a>}</div></article>)}</div></div></section>;
  ...
  if (section.type === "timeline") return <section className="sb-section sb-timeline-section" style={vars}><div className="sb-inner"><div className="sb-section-head"><span className="sb-kicker">{section.eyebrow}</span><h2>{section.title}</h2></div><div className="sb-timeline"><div className="sb-drive-line"/><div className="sb-drive-arrow"/>{section.items.map((item,index)=><article className={index===section.items.length-1?"next":""} key={item.id}><strong>{item.title}</strong><span className="sb-ball"><img src={footballIcon} alt=""/></span><small>{item.subtitle}</small>{item.value&&<div className="sb-points"><span>POINTS</span><b>{item.value}</b></div>}{item.icon&&<img className="sb-timeline-icon" src={item.icon} alt=""/>}<p>{item.text}</p></article>)}</div></div></section>;

  if (section.type === "cta") return <section className="sb-section sb-cta" style={vars}><div className="sb-inner"><span className="sb-kicker">{section.eyebrow}</span><h2>{section.title} {section.accent&&<i>{section.accent}</i>}</h2><p>{section.text}</p><LinkButton label={section.buttonLabel} url={section.buttonUrl}/></div></section>;
...
export function SiteBuilderPage({ state, page }: { state: SiteBuilderState; page: BuilderPage }) {
  ...
  return <div className="sb-site" style={{"--site-bg":state.theme.background,"--site-text":state.theme.text,"--site-accent":state.theme.accent,"--site-surface":state.theme.surface,"--site-muted":state.theme.muted} as CSSProperties}><BuilderHeader state={state} current={page}/><main>{page.sections.map((section)=><BuilderSectionView key={section.id} section={section}/>)}</main>{hasGames&&<DynamicHomeGames/>}{hasFeeds&&<DynamicHomeFeeds/>}<footer className="sb-footer"><span>HELLENSTEIN RASCALS</span><small>American Football · Heidenheim</small></footer></div>;
}

export { BuilderSectionView };

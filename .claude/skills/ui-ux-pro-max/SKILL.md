...
**Available stacks in this vendored copy:** `react`, `nextjs`, `html-tailwind`, `shadcn` (trimmed to this project's stack; the upstream skill also ships `vue`, `svelte`, `astro`, `nuxtjs`, `nuxt-ui`, `angular`, `laravel`, `swiftui`, `react-native`, `flutter`, `jetpack-compose`, `threejs`, `javafx`, `wpf`, `winui`, `avalonia`, `uno`, `uwp` — see `THIRD_PARTY_NOTICES.md` for the source repo if one of those is needed later). Use the stack detected in Step 1.
